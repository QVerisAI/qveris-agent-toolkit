...
| 执行结果 | Call | `success`、`error_message`、`execution_outcome` | 解释 provider/result 是否可用。不要只用 `success` 判断最终是否扣费。 |
| 预结算账单 | Call 和调用审计 | `billing`、`pre_settlement_bill`、`requested_amount_credits` | 展示最终结算、折扣或不扣费规则应用前的请求金额。 |
...
2. 调用能力并保存 `execution_id`、`billing`、`execution_outcome` 和旧字段 `cost`。
...
- REST 客户端应保留 Call 响应中的 `execution_id`、`billing`、`execution_outcome` 和 `cost`。
- CLI、MCP 和 SDK 客户端应展示同一组机器可读字段，不要翻译成不相关的本地状态名。
- 自动化逻辑优先使用 `charge_outcome`、`reason_code` 等稳定机器字段；面向用户的展示可使用 `billing_summary` 和 `error_message`。
...
| `execution_outcome` | object | 可用时返回结构化结果和计费结果上下文。包含 `outcome`、`reason_code`、`provider_success`、`billable_success`、`result_valid` 等稳定字段。 |
...
| 第三方接口错误 | 参数已接受，但上游返回 HTTP/provider 错误。 | `execution_outcome.reason_code` 是否以 `provider.` 开头？ | 视情况重试、选择其他 provider，或带 `execution_id` 联系支持。 |
...
[QVeris 公开 OpenAPI](https://qveris.ai/openapi/qveris-public-api.openapi.json) 文档包含 Discover、Inspect 和 Call 路径的请求体、响应结构与示例。稳定的投影与旧服务兼容样例见[投影 fixtures](https://qveris.ai/openapi/qveris-public-api.projection-fixtures.json)。
