...
`session_id` 可选。建议每个用户任务或会话使用一个稳定值，用于追踪、分析和 pricing context。它不是缓存合同，也不承诺缓存复用或 `session_cache_hit`。
...
| `session_id` | string | 否 | 当前用户任务的追踪和 pricing-context id |
  ...
  "results": [
    {
      ...
      "expected_cost": "每次成功请求 5 credits",
      "billing_rule": {
        "unit": "request",
        "amount_credits": 5
      },
...
| `remaining_credits` | number/null | 可用时返回账户剩余 credits。 |
...
| `session_id` | string | 否 | 追踪和 pricing-context id；省略时服务可能使用 execution id |
    ...
    "items": [
      {
        ...
        "billing_summary": "每次成功请求 5 credits",
