...
QVeris 能力路由网络的官方命令行工具。直接在终端或 Agent 框架中发现、检查和调用 10,000+ API 能力。
...
使用自然语言搜索 API 能力。返回工具名称、提供商、ID、描述、相关性得分、成功率、延迟和预估成本。
...
使用 QVeris API key 认证。打开浏览器获取 key，然后掩码输入。
...
自检诊断：验证 Node.js 版本、API key 配置和 API 连通性。
