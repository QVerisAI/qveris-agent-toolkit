...
| `--max-credits <n>` | Not supported for a context Call: the exact-tool execution endpoint has no server-enforced budget field | — |
...
Before any Call, the CLI performs a fresh Discover and uses only its new `search_id`. An exact-tool context uses the `tool_id` alone as the deterministic query. A service-only context uses task/service intent, returns refreshed candidates, and stops; it never guesses a tool. A complete Discover schema can complete parameter preflight. Inspect is used only when required contract detail is missing, and Probe only when schema/parameter validation, permission/pricing confirmation, explicit `--require-quote`, a budget cap, or paid risk requires it.

Expiry invalidates the old availability, price, and permission snapshots, not safe task intent or public IDs; the CLI automatically refreshes instead of terminating. Any present non-free or ambiguous price signal, including human-readable `expected_cost`, is paid risk and requires a structurally valid credits quote. A Probe quote never reserves price or authorizes execution, so `--max-credits` is rejected for exact-tool context calls until their execution route accepts a server-enforced cap. Missing side-effect or idempotency metadata is reported as a warning and keeps a paid Call strict single-submit; only explicitly reported dangerous side effects or non-idempotency require their matching confirmation flags. Context validation uses a bounded iterative traversal, so hostile deep nesting returns a structured rejection instead of exhausting the call stack.

Safe recovery is automatic where the published contract provides enough proof: stale discovery is refreshed immediately, an incomplete schema triggers Inspect, and a missing exact tool triggers service rediscovery. A fallback is auto-selected only when exactly one current candidate reports the same explicit `service_id`; ambiguous or unproven candidates still require selection. A submitted Call is never replayed automatically. If an error carries an `execution_id`, the CLI performs one bounded Usage/Ledger reconciliation and reports unknown settlement when no final record exists. JSON results and failures include a stable `next_action` object, so agents do not need internal digest, evidence, or version details.
