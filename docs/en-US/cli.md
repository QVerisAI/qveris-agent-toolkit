...
Expiry invalidates the old availability, price, and permission snapshots, not safe task intent or public IDs; the CLI automatically refreshes instead of terminating. Any present non-free or ambiguous price signal, including human-readable `expected_cost`, is paid risk and requires a structurally valid credits quote. `--max-credits` also requires `exact: true`; an inexact estimate cannot guarantee a hard cap. When no price signal exists, unknown price is reported as a structured warning and execution may continue. Authentication, JSON parameter types (including integer versus number), required parameters, budget, explicit permission/region prohibitions, dangerous side effects, and idempotency metadata are checked fail-closed immediately before Call.
