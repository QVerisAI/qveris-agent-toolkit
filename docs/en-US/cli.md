...
Before any Call, the CLI performs a fresh Discover and uses only its new `search_id`. An exact-tool context uses the `tool_id` alone as the deterministic query. A service-only context uses task/service intent, returns refreshed candidates, and stops; it never guesses a tool. A complete Discover schema can proceed directly to execution confirmation. Inspect is used only when required contract detail is missing, and Probe only when schema/parameter validation, permission/pricing confirmation, explicit `--require-quote`, a budget cap, or paid risk requires it.

Expiry invalidates the old availability, price, and permission snapshots, not safe task intent or public IDs; the CLI automatically refreshes instead of terminating. Any present non-free or ambiguous price signal, including human-readable `expected_cost`, is paid risk and requires a structurally valid credits quote. A Probe quote never reserves price or authorizes execution, so `--max-credits` is rejected for exact-tool context calls until their execution route accepts a server-enforced cap. When no price signal exists, unknown price is reported as a structured warning and execution may continue. Authentication, JSON parameter types (including integer versus number), required parameters, explicit permission/region prohibitions, dangerous side effects, and idempotency metadata are checked fail-closed immediately before Call. Context validation uses a bounded iterative traversal, so hostile deep nesting returns a structured rejection instead of exhausting the call stack.
