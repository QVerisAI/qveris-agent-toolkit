...
<!-- benchmark-cadence:0bd5c3d4d716b6d6abfd6bd8b91cd5846b115778 -->
## 2026-09-09 — coordinated release `0bd5c3d4d716`

This baseline was generated by a reviewed maintainer-run release benchmark for
`cli-v0.11.3`, `mcp-v0.14.4`, `js-sdk-v0.8.4`, `python-sdk-v0.7.3`. It uses
immutable `tasks/v4.jsonl`, 18 tasks, 3 trials per task, Top 10 discovery, and
real calls. Failed trials remain in the denominator.

| Metric | Curated reference route | `gpt-5.6-sol` configured model |
| --- | ---: | ---: |
| Runs | 54 | 54 |
| Completed and executed | 48/54 | 54/54 |
| Selection grounded | 88.89% | 100.00% |
| Inspection grounded | 88.89% | 100.00% |
| Required-parameter accuracy | 100.00% | 100.00% |
| Constraint accuracy | 88.89% | 90.74% |
| Call success among attempted calls | 97.92% (47/48) | 100.00% (54/54) |
| Result non-empty among successful calls | 100.00% (47/47) | 100.00% (54/54) |
| Strict workflow success | 87.04% (47/54) | 90.74% (49/54) |
| Task-cluster bootstrap 95% interval | 70.37%–100.00% | 77.78%–100.00% |

The strict benchmark gap is **-2/54 = -3.70 percentage points**. This is an
end-to-end benchmark difference, not automatically a pure routing effect. The
negative sign means the configured model recorded the higher strict success
rate in this sample. The lanes observed catalog
digests `fd84049b6119d8658c3485caa4029b8bcad1ef4e032e26e66f7613c445037cf5` and
`e534593cc6b33b46e707efa23c9f43a78fc50044795f0be2c491681afc2f8e21`; reviewers must confirm
whether the API and catalog revisions support any stronger comparison claim.
The configured provider model revision is `unreported`, so it must not be
described as a pinned-model snapshot.

Both lanes observed API revision `2026-09-07.1`; the API reported no catalog
revision, and the sequential lanes recorded different catalog-observation
digests. The reference route missed its fixed Top-10 candidate for all three
`timezone-tokyo` and all three `market-holidays` trials; one
`geocode-address` Call returned a failure envelope. The configured model used
Codex CLI `0.144.1` with medium reasoning, grounded every selection and
inspection, and completed 54/54 successful non-empty Calls. Its five strict
failures were constraint mismatches: all three `timezone-tokyo` trials, one
`weather-london` trial, and one `domain-intel` trial. Successful execution does
not override those parameterization failures.

Artifacts:

- `2026-09-09-reference-v1-release-0bd5c3d4d716-v4.runs.jsonl`
- `2026-09-09-reference-v1-release-0bd5c3d4d716-v4.summary.json`
- `2026-09-09-gpt-5.6-sol-configured-release-0bd5c3d4d716-v4.runs.jsonl`
- `2026-09-09-gpt-5.6-sol-configured-release-0bd5c3d4d716-v4.summary.json`
...
| Historical wrapper-level non-empty observation | 100% (51 / 51) | 100% (45 / 45) |
...
| Historical wrapper-level non-empty observation | 100% (51 / 51) | 100% (46 / 46) |
