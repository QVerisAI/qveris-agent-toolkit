...
export function buildCadencePlan({
  ...
  const referenceAdapterRevision = `${releaseSha}/reference-v1`;
  const configuredAdapterRevision = `${releaseSha}/codex-cli-${config.configured_model.cli_version}/${config.configured_model.reasoning_effort}`;
  return {
    releaseSha,
    shortSha,
    ...
    referenceAdapterRevision,
    configuredAdapterRevision,
...
function percent(value) {
  ...
  if (typeof value !== 'number' || !Number.isFinite(value) || value < 0 || value > 1) {
    throw new Error('Benchmark rates must be finite numbers from 0 to 1 or null');
  }
...
function resultNonemptyMetric(model, successfulCalls, label) {
  if (successfulCalls === 0) return 'N/A (no successful calls)';
  if (model.result_nonempty_rate === null) {
    throw new Error(`${label} summary is missing result non-emptiness evidence for successful calls`);
  }
  const nonempty = Math.round(model.result_nonempty_rate * successfulCalls);
  return `${percent(model.result_nonempty_rate)} (${ratio(nonempty, successfulCalls)})`;
}
...
function validateSummaryForPlan(summary, plan, lane) {
  ...
  const expectedAdapterRevision =
    lane === 'reference' ? plan.referenceAdapterRevision : plan.configuredAdapterRevision;
...
export function buildResultSection({ referenceSummary, configuredSummary, plan, generatedAt }) {
  ...
  const referenceNonempty = resultNonemptyMetric(reference, referenceCallSuccesses, 'Reference');
  const configuredNonempty = resultNonemptyMetric(configured, configuredCallSuccesses, 'Configured-model');
  const gapDirection =
    gap < 0
      ? 'The negative sign means the configured model recorded the higher strict success rate in this sample.'
      : 'A non-negative value means the curated reference route recorded an equal or higher strict success rate in this sample.';

  return `<!-- benchmark-cadence:${plan.releaseSha} -->
## ${generatedDate} — coordinated release \`${plan.shortSha}\`

This baseline was generated by a reviewed maintainer-run release benchmark for
${plan.releaseTags.map((tag) => `\`${tag}\``).join(', ')}. It uses immutable \`${plan.taskSet.replace('benchmarks/discover-call/', '')}\`,
${reference.tasks} tasks, ${reference.trials_per_task} trials per task, Top ${referenceMetadata.discovery_limit} discovery, and real calls.
Failed trials remain in the denominator.
...
| Result non-empty among successful calls | ${referenceNonempty} | ${configuredNonempty} |
...
The strict benchmark gap is **${ratio(Math.round(gap * reference.runs), reference.runs)} = ${(gap * 100).toFixed(2)} percentage points**. This is an end-to-end benchmark
difference, not automatically a pure routing effect. ${gapDirection} The lanes observed catalog
...
The configured provider model revision is \`${configuredMetadata.model_revision}\`, so a value of \`unreported\`
must not be described as a pinned-model snapshot.
...
function planCommand(options) {
  ...
  const values = {
    ...
    reference_adapter_revision: plan.referenceAdapterRevision,
    ...
    configured_adapter_revision: plan.configuredAdapterRevision,
  ...
  if (options.githubOutput) writeGithubOutput(options.githubOutput, values);
  else process.stdout.write(`${JSON.stringify(values, null, 2)}\n`);
}
